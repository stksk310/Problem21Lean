...
/-- Precise remaining MINBOX obligation. The least actual level is supplied by a proved
minimum theorem; no arm existence, White conclusion, or k=1 assertion is a field of data. -/
def MinimumOneStatement : Prop :=
  ∀ (g : Generators) (_s : g.Setting) (D : HerzogCriticalData g) (colorA : Bool)
    (k : ℕ) (p : Point),
    0 < k →
...
theorem three_arms_impossible_of_residuals
  ...
  · simp only [Bool.false_eq_true, ↓reduceIte] at hcap harms
    ...
    have hk1 := hminimum g s D false k p hk
  ...
  · simp only [↓reduceIte] at hcap harms
    ...
    have hk1 := hminimum g s D true k p hk
      (fun i => ⟨(hp i).1,lt_of_le_of_lt (hp i).2 (hcap i)⟩) he hmin
    subst k
    simp only [Nat.cast_one,one_mul] at he
