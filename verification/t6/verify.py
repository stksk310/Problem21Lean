"""Fresh T6 verification and evidence generation."""
from pathlib import Path
import hashlib, json, os, re, subprocess, sys

ROOT = Path(__file__).resolve().parents[2]
HERE = ROOT / "verification/t6"
EVIDENCE = ROOT / "audit-evidence/t6"
BASE = "707d9386037e8bcdec9c9bfb02f95731c9ce2597"
...
def modules():
    ...
    paths=sorted((ROOT/"P21/Nonsymmetric/TypeII").glob("*.lean"))
    paths.append(ROOT/"P21/Nonsymmetric/TypeII.lean")
    mods=[p.relative_to(ROOT).with_suffix("").as_posix().replace("/",".") for p in paths]
    write("T6_MODULE_LIST.txt", "\n".join(mods)+"\n")
    return mods

def debt():
    forbidden=re.compile(r"\b(sorry|admit|axiom|sorryAx|run_tac|native_decide)\b|\bunsafe\s+(?:theorem|def)\b|\bopaque\b")
    hits=[]
    paths=list((ROOT/"P21/Nonsymmetric/TypeII").glob("*.lean"))+[ROOT/"P21/Nonsymmetric/TypeII.lean"]
...
def dag():
    ...
    root="P21.Nonsymmetric.TypeII.Exclusion"; seen=set()
    def collect(n):
        if n in seen:return
        seen.add(n)
        for c in graph.get(n,[]):collect(c)
    collect(root)
    forbidden={"P21.Nonsymmetric.TypeII.Integration","P21.Nonsymmetric.Path.Exclusion",
               "P21.Nonsymmetric.Path.Integration","P21.Nonsymmetric.ColorCap.FullColorCap"}
    bad=sorted(seen&forbidden)
    write("DEPENDENCY_DAG_CHECK.txt",json.dumps(graph,indent=2)+"\nIMPORT DAG: ACYCLIC\n")
    write("CIRCULARITY_CHECK.txt","T6 exclusion imports:\n"+"\n".join(sorted(seen))+"\nFORBIDDEN="+repr(bad)+"\n")
...
def main():
    ...
    run(lake()+["build",*mods],"T6_BUILD_LOG.txt")
    run(lake()+["env","lean","verification/t6/StatementGate.lean"],"STATEMENT_INSPECTION.txt")
    ax=run(lake()+["env","lean","verification/t6/GeneratedAxiomCheck.lean"],"AXIOM_REPORT.txt")
    ...
    run(lake()+["env","lean","verification/t6/StatementGate.lean"],"T6_VERIFICATION_LOG.txt")
    run(lake()+["env","lean","verification/t6/Regression.lean"],"T6_VERIFICATION_LOG.txt",append=True)
